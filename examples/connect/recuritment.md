...
基于 `lark-cli mail` 操作 HR 邮箱（hr@example.com）的经验总结。
...
操作 HR 邮箱时指定 `--mailbox hr@example.com`。
...
```bash
# 搜索含关键词的邮件
lark-cli mail +triage --mailbox hr@example.com --query "笔试题" --max 100
lark-cli mail +triage --mailbox hr@example.com --query "笔试答案" --max 100
lark-cli mail +triage --mailbox hr@example.com --query "作答" --max 100
```
...
1. **查已发送**：搜索 SENT 文件夹中 HR 发出的笔试题邮件，获取收件人地址
   ```bash
   lark-cli mail +triage --mailbox hr@example.com --filter '{"folder":"SENT"}' --query "笔试题" --max 100
   ```

2. **看具体邮件**：查看某封邮件的收件人
   ```bash
   lark-cli mail +message --mailbox hr@example.com --message-id <id>
   ```

3. **查回复**：搜索 INBOX 中该收件人是否有回复
   ```bash
   lark-cli mail +triage --mailbox hr@example.com --query "<邮箱地址>" --max 20
   ```
...
- **连招示例**：`--mailbox hr@example.com` + `--filter '{"folder":"SENT"}'` 是查看已发送邮件的标准组合
...
```bash
# 搜岗位关键词
lark-cli mail +triage --mailbox hr@example.com --query "<岗位>" --max 100

# 看某封申请邮件的正文（含附件）  
lark-cli mail +message --mailbox hr@example.com --message-id <id>

# 查 HR 回复了什么
lark-cli mail +triage --mailbox hr@example.com --filter '{"folder":"SENT"}' --query "<申请人关键词>" --max 20
```
