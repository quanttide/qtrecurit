...
- exam 模板添加姓名变量，支持个性化考核邀请
- exam 命令添加自动归档到已发送笔试文件夹功能
- 投递邮件搜索逻辑优化：优先从「已发送实训邀请」文件夹搜索，未找到则从收件箱搜索
...
- 新增二维码缓存功能（`cache set-qr`/`show-qr`/`clear-qr`）
- 邀请邮件发送时自动使用缓存的二维码作为附件
- 支持 `--qr` 参数手动指定二维码路径（优先级高于缓存）
- 发送前自动切换工作目录处理 lark-cli 绝对路径限制

### Docs

- 更新 send-invite-email.md 文档
- 补充批量发送脚本示例
- 补充常见问题处理（并发冲突、邮箱缺失、二维码过期）
- 添加 ROADMAP.md 记录命令打磨计划
...
- 移除发送日志写入，信息直接打印到控制台
- 模板目录移至 src/cli/templates/，与源码同级
- 使用 include_str! 编译时嵌入模板，移除运行时文件加载
